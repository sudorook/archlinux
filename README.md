...
Install Arch Linux on a new block device (GPT paritioning scheme). Supports
...
#### 9) Personalization
```
1) Back                             10) Import GNOME terminal profiles
2) All                              11) Enable autologin
3) Select system fonts              12) Invert brightness (i915)
4) Select icon theme                13) Enable IOMMU (Intel)
5) Select desktop theme             14) Disable PulseAudio suspend
6) Set dark GTK                     15) Disable 802.11n
7) Import Cinnamon dconf            16) Add scripts
8) Import GNOME dconf               17) Select default kernel
9) Import application dconf
```
...
17) Select the default boot kernel from the currently installed ones.
